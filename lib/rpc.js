...
const DEFAULT_QUERY = Object.freeze(Object.create(null));
...
class RPC extends EventEmitter {
  ...
  async call(body, query) {
    ...
    for (const cmd of cmds) {
      ...
      let result;
      try {
        result = await this.execute(cmd, false, query);
      } catch (err) {
  ...
  async execute(json, help = false, query = DEFAULT_QUERY) {
    const func = this.calls[json.method];

    if (!func) {
      for (const mount of this.mounts) {
        if (mount.calls[json.method])
          return await mount.execute(json, help, query);
      }
      throw new RPCError(errors.METHOD_NOT_FOUND,
        `Method not found: ${json.method}.`);
    }

    return func.call(this, json.params, help, query);
  }
